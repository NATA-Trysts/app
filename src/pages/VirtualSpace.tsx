...
const VirtualSpace = () => {
  ...
  useEffect(() => {
    auth && setUser({ ...user, username: auth.user.username, handler: auth.user.handler })
  }, [auth])
        ...
        <Container customColor={customColor}>
          ...
          <OverlayContainer>
            ...
            <LeftSideContainer>
              <LeftSideWrapper>
                <MyVideo />
                <MyInformationCard avatar={user.avatar.image} handler={user.handler} name={user.username} />
              </LeftSideWrapper>
            </LeftSideContainer>
            <MiddleSideContainer>
              <ToolbarMiddle />
